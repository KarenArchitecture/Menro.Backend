...
//                    /* -------------------------
//                       Restaurant categories (all CustomFoodCategory)
//                       - Some are based on GlobalFoodCategory (linked via GlobalCategoryId)
//                       - Some are brand-new custom ones
//                    ------------------------- */
//                    var catCount = rand.Next(MinCatsPerRestaurant, MaxCatsPerRestaurant + 1);

//                    for (int c = 0; c < catCount; c++)
//                    {
//                        bool basedOnGlobal = rand.NextDouble() < 0.6; // 60% of categories come from global ones
...
//                            {
//                                Name = globalCat.Name,                   // reuse global name
//                                IconId = globalCat.IconId,               // reuse admin-uploaded icon
//                                RestaurantId = restaurant.Id,
//                                GlobalCategoryId = globalCat.Id          // <-- key change: link to global
//                            };
//                        }
//                        else
//                        {
//                            // fully custom category
//                            customCat = new CustomFoodCategory
//                            {
//                                Name = $"دسته ویژه {c + 1}",
...
//                                RestaurantId = restaurant.Id,
//                                CustomFoodCategoryId = customCat.Id, // <--- now all foods go through custom categories
//                                ImageUrl = FoodFallbackImage,
    ...
    public class DbInitializer : IDbInitializer
    {
        ...
        public async Task InitializeAsync()
        {
            try
            {
                ...
                /* ============================================================
                   Variants + Addons (moved outside loop)
                ============================================================ */
                var seededFoods = await _db.Foods.ToListAsync();
                foreach (var food in seededFoods)
                {
                    if (rand.NextDouble() < 0.55)
                    {
                        ...
                        var variants = new List<FoodVariant>
                        {
                            new() { Name = "کوچک", Price = basePrice - (int)Math.Round(basePrice * 0.15), FoodId = food.Id },
                            new() { Name = "متوسط", Price = basePrice, FoodId = food.Id }
                        };
                        if (rand.NextDouble() < 0.6)
                            variants.Add(new FoodVariant { Name = "بزرگ", Price = basePrice + (int)Math.Round(basePrice * 0.2), FoodId = food.Id });
                        ...
                        foreach (var v in variants)
                        {
                            ...
                            for (int a = 0; a < addonCount; a++)
                            {
                                _db.FoodAddons.Add(new FoodAddon
                                {
                                    Name = a switch
                                    {
                                        0 => "پنیر اضافه",
                ...
                foreach (var r in allRestaurants)
                {
                    ...
                    if (discountedFoods.Any())
                    {
                        ...
                        r.Description += $" 🌟 بیشترین تخفیف: {maxDiscount}%";
                    }
                }
                await _db.SaveChangesAsync();
